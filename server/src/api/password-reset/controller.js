...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Trekservice account.<br>
        ...
        &mdash; Trekservice Team
      `
      return sendMail({ toEmail: email, subject: 'Trekservice - Password Reset', content })
    })
